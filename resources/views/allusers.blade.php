            ...
            <div class="container position-relative px-4 px-lg-5">
                <div class="row gx-4 gx-lg-5 justify-content-center">
                    <div class="col-md-10 col-lg-8 col-xl-7">
                        ...
                        <center><a href="{{route('posts.create')}}" class="btn btn-primary">Dodaj novi post</a>
                            
                            
                            <a href="{{url('posts/all')}}" class="btn btn-secondary">Svi postovi</a>

                            
                            
                            <h1 style="margin-top:10px;">Svi korisnici</h1></center>
...
<?php $i=1;?>
                    ...
                    <td>{{$user->name}}</td>
                    <td>{{$user->email}}</td>
                    <td><img style="width:100px;height:100px;" src="{{asset('storage/slike/'.$user->picture)}}" alt=""></td>
                    <td>{{$user->about}}</td>
                    <td>{{$user->created_at}}</td>
                    <td>{{$user->updated_at}}</td>
                    <td class="akcije">
                    <a href="{{url('/posts/user/'.$user->id)}}" class="btn btn-primary">View</a>
                    @if ( auth()->user() && auth()->user()->is_admin == 1 && $user->is_admin < 1 )
                    <form action="{{url('user/delete/'.$user->id)}}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" name="submit" class="btn btn-danger btn-outline-secondary" style="border-radius:0;color:black;">Izbrisi korisnika</button>
                    @endif
                  </form>
                    </td>
                    </tr>
